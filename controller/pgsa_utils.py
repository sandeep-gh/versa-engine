...
def get_db_host_type(dbdesc=None, wdir=None):
    session_fn = get_session_fn(dbdesc)
...
def get_db_port(dbdesc=None, wdir=None):
    ''' 
...
def get_db_data_home(dbdesc=None, user=None, port=None):  # why is there dbdesc here
    assert(dbdesc is not None)
    assert(user is not None)
    if port is None:
        port = get_db_port(dbdesc)
    # TODO: the base needs to be a config parameter
    return Template("/media/kabira/home/var/${user}/pgsql_sa_${port}/").substitute(locals())
