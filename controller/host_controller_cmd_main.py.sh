...
if args.startdb:
    cntrl.initdb(dbdesc=dbsession_dbdesc, host_type=host_type, log_statement=log_statement, db_cfg_param_val_dict = db_cfg_param_val_dict)

if args.restartdb:
    cntrl.startdb(dbsession_dbdesc, host_type=host_type, log_statement=log_statement, db_cfg_param_val_dict= db_cfg_param_val_dict)


if args.cleandb: ## stop and remove files
    cntrl.cleanup(dbdesc=dbsession_dbdesc)
    

if args.savedb:
    cntrl.savedb(dbdesc=dbsession_dbdesc)

if args.restoredb:
    cntrl.resumedb(dbsession_dbdesc)



    
if args.stopdb:
    cntrl.stopdb(dbsession_dbdesc)
...
if args.pingdb:
    cntrl.pingdb(dbsession_dbdesc, args.iambash)
