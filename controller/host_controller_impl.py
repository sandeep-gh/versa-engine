...
def initdb(db_data_home, port, allow_connections_from_subnet='True', db_cfg_param_val_dict=None, log_statement='none'):
    ...
    initdb_cmd_str = Template(
        "mkdir ${db_data_home};initdb -E UTF8 -D ${db_data_home}/data").substitute(a)
    initdb_cmd_str = initdb_cmd_str
...
def startdb(db_data_home, port=None, db_cfg_param_val_dict=None, log_statement='none'):
    ...
    default_db_cfg_param_val_dict['shared_buffers'] = "4GB"
    default_db_cfg_param_val_dict['maintenance_work_mem'] = "5GB"
    default_db_cfg_param_val_dict['work_mem'] = "2GB"
    default_db_cfg_param_val_dict['effective_cache_size'] = "10GB"
    default_db_cfg_param_val_dict['max_wal_size'] = "4GB"
    default_db_cfg_param_val_dict['checkpoint_timeout'] = "30min"
...
def stopdb(db_data_home):
    stopdb_cmd_template = Template("pg_ctl -D ${db_data_home}/data stop ")
    a = locals()
    b = globals()
    a.update(b)
    stopdb_cmd_str = stopdb_cmd_template.substitute(a)
    #stopdb_cmd_str = bash_prefix  + " " + stopdb_cmd_str
    # print stopdb_cmd_str
    subprocess.call(stopdb_cmd_str, shell=True)

    # save the database image on target_node
