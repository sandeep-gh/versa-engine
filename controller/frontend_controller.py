...
def launch_frontend_proxy(jobmanager, frontend_proxy_port=None):
    curr_dir = os.getcwd()
...
def launchdbjob(dbdesc=None, dbport=None, save_on_exit_val="no", postjob_cleanup=False, host_type='standard', log_statement='all', run_mode='default', walltime_hours=1, jobmanager=None):
    '''
    ...
    db_data_home = pgu.get_db_data_home(
        dbdesc=dbdesc, user=dbuser, port=dbport)

    # TODO: use path based on pgversion

    cluster_name = AppConfig.get_cluster_name()
    start_cmd = Template(
        "--dbsession ${dbdesc} --run_mode ${run_mode} --host_type ${host_type} --log_statement ${log_statement} --startdb ").substitute(locals())
    ping_cmd = Template(
