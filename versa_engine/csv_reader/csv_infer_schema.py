...
def forward_reader(fn_or_bytes):
    '''
    ...
    try:
        if isinstance(fn_or_bytes, str):
            return open(fn_or_bytes, encoding="utf-8")
        if isinstance(fn_or_bytes, bytes):
            bp = BytesIO(fn_or_bytes)
            bp.seek(0, io.SEEK_SET)
            return io.TextIOWrapper(bp, encoding='utf-8')
    except UnicodeDecodeError:
        print("UnicodeDecodeError..try gzip ")

        
def reverse_reader(fn_or_bytes):
    if isinstance(fn_or_bytes, str):
        bp =  fn_or_bytes
    if isinstance(fn_or_bytes, bytes):
        bp = BytesIO(fn_or_bytes)
        bp.seek(0, io.SEEK_SET)
    return FileReadBackwards(bp, encoding="utf-8")
...
def get_csv_report(csvstore):
    ...
    _flr = forward_reader(csvstore)
    _rlr = reverse_reader(csvstore)
...
def build_csv_metadata(model_name=None,  csv_metadata=None):
    ...
    _ = _.replace('&', '&amp;') #escape ampersend if any -- should be replaced much earlier
