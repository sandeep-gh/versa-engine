...
def get_cols_nametype(session, object):
    all_attrs = []
    robject = object
    if(type(object) == sqlalchemy.ext.declarative.DeclarativeMeta):
        robject = session.query(object).subquery()
    for col in getColNames(session, robject):
        all_attrs.append((col, getattr(robject.c, col).type))
    return all_attrs
...
def get_pkeys(session, object):
    ...
    if(type(object) == sqlalchemy.ext.declarative.DeclarativeMeta):
        robject = session.query(object).subquery()
    pkeys = [col  for col in cols if getattr(robject.c, col).primary_key]
    return pkeys
