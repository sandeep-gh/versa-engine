...
def get_latest_dbsession(work_dir="./"):
    ...
    """
    lru_dbsession = utilities.get_last_file_by_pattern(
        "dbsession*.py", work_dir)  # lru: least recently used
...
def build_session(dbsession=None, port=None, conn_remote=False, session_run_dir='./'):
    ...
    # TODO: connection string should be build through a function call
    conn_str = 'postgresql+psycopg2://'+username + \
        '@' + dbhost + ':' + str(port) + '/postgres'
