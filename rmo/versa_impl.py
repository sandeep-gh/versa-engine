...
def append_import_header(tbl_name):
    #####################
    # add to runner
    ####################
    import_header = open('.x.py', 'a')
    print("from ", tbl_name + '_model import *', file=import_header)
    import_header.close()
    cmd_template = Template('sort -n .x.py | uniq > import_header.py')
    a = locals()
    b = globals()
    a.update(b)
    cmd_str = cmd_template.substitute(a)
    subprocess.call(cmd_str, shell=True)


def build_orm_from_metadata(metadata_root, base_tbl_name=None, create_table=False, force_create_model=True, location=None, class_decorator=""):
    ...
    if base_tbl_name is not None:
        ...
        create_pg_tbl_cmd = Template(
            "awk -f ${module_dir}/create_view.awk ${work_dir}/${model_name}.schema | sed \'s/__TBL__/\'$base_tbl_name\'/g\' | sed \'s/__VIEW__/\'${tbl_name}\'/g\' | psql -p $port postgres  >> ${work_dir}/psql_out")
        a = locals()
        b = globals()
        a.update(b)
    ...
    subprocess.call("echo \"from rmo.header import * \" > " +
                    model_name+"_model.py", shell=True)

    #create orm###
    create_orm_cmd = Template("awk -f ${module_dir}/create_orm_from_schema.awk ${work_dir}/${model_name}.schema  | sed \'s/__MODEL__/\'${model_name}\'/\' | sed \'s/__TBL__/\'${model_name}\'/\' | sed \'s/__PKEY_STRING__/\'${primary_key}\'/\' | sed \'s/__CLASS_DECORATOR__/\'${class_decorator}\'/\'| sed \'s/__FOREIGN_KEYS_STRING__/\'${foreign_keys_string}\'/\' >> ${model_name}_model.py")
    a = locals()
    b = globals()
    a.update(b)
...
def build_only_orm_from_metadata(metadata_root, base_tbl_name=None, create_table=False, force_create_model=True, location=None, class_decorator=""):
    model_name = mu.get_model_name(metadata_root)
    if create_table:
        dei.create_table_from_metadata(metadata_root, wd=work_dir)
    ...
    for fe in all_files:
        ...
        if vu.check_model_exists(model_name) and not force_create_model:
            ...
            build_only_orm_from_metadata(
                metadata_root, force_create_model=force_create_model)
...
def build_orms(cfg_xml, force_create_model=False, depth=0):
    '''
    ...
    if xu.check_if_xml_tree(cfg_xml):
        ...
        croot = ecu.read_config_file(cfg_xml)
    build_orm_files(croot, force_create_model=force_create_model)
    if xu.has_key(croot, 'edconfig', path_prefix='./'):
        ...
        for edcfg_elem in edcfg_elems:
            ...
            build_orms(
                edcfg_elem, force_create_model=force_create_model, depth=depth+1)


def build_orm_files(cfg_xml, datasets=None, force_create_model=False):
    if xu.check_if_xml_tree(cfg_xml):
        croot = cfg_xml
    else:
        croot = ecu.read_config_file(cfg_xml)
    ...
    for fe in all_files:
        ...
        if vu.check_model_exists(model_name) and not force_create_model:
            ...
            if filetype == 'csv':
                dei.create_table_from_metadata(metadata_root, wd=work_dir)
                dei.load_csv_file(metadata_root, file_locations, model_name=model_name,
...
def build_orm_oracle_tables(cfg_xml, make_local_copy=False, force_create_model=False):
    '''
    ...
    if remote_tbls is not None:
        for remote_tbl in remote_tbls:
            ...
            if vu.check_model_exists(model_name) and not force_create_model:
                ...
                build_orm_from_metadata(
                    metadata_root, base_tbl_name=base_tbl_name)
...
def build_orm_pg_tables(cfg_xml, force_create_model=False, make_local_copy=False):
    ...
    for remote_tbl in remote_tbls:
        ...
        if vu.check_model_exists(model_name) and not force_create_model:
            ...
            build_orm_from_metadata(
                metadata_root, create_table=False, base_tbl_name=base_tbl_name)
