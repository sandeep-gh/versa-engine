...
parser.add_argument("--build_rmo", action='store_true',
                    help="don't ingest the data; only build the rmo wrapper")
...
if args.data_config:
    ...
    vi.build_orms(edcfg_xml,  force_create_model=force_create_model)

    # currently automatically copying local data from remote source is turned off
    vi.build_orm_oracle_tables(
        edcfg_xml, make_local_copy=make_local_copy, force_create_model=force_create_model)

    # TODO: we need to fix this
    vi.build_orm_pg_tables(
        edcfg_xml, force_create_model=force_create_model, make_local_copy=make_local_copy)
