...
def load_csv_file(metadata_xml, locations, wd=None, dbport=None, pgdb='postgres', model_name=None, schema='public', delimiter=None, strict_formatted=False, has_header=False, crop_head=None):
    ...
    for fe in all_files:
        ...
        if location is not None:
            lfi.ingest_from_file(metadata_cfg=metadata_root, cursor=cursor, ddir=os.path.dirname(location), fn=os.path.basename(
                location), work_dir=work_dir, port=port, delimiter=delimiter, strict_formatted=strict_formatted, crop_head=crop_head)
